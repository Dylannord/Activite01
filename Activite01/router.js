...
exports.dispatch_API_EndPoint = function(req, res){
    ...
    if (API_Endpoint_Ok(req.url)) {
        ...
        try{
			...
			let stringParam = controller.getQueryStringParams();
            // to do : find methods that contain the http verb
            if (req.method === 'GET') {
                if(stringParam != null)
				{
					if("name" in stringParam)
					{
						controller.getName(req, res);
					}
					else if("category" in stringParam)
					{
						controller.getCategory(req, res);
					}
					else if(stringParam.sort == '"name"' && "sort" in stringParam){
						
						controller.sortName();
					}
					else if(stringParam.sort == '"category"' && "sort" in stringParam){
